        ...
        fun handleIntent(intent: EventIntent) {
            ...
            viewModelScope.launch {
                when (intent) {
                    ...
                    is EventIntent.JoinMeeting -> _sideEffect.send(OpenMeetingUrl(intent.meetingUrl))
                    ...
                    is ToggleEventAlarm -> toggleEventAlarmUseCase(intent.event, intent.enabled, intent.allOccurrences)
                    is ToggleEventVibrate -> toggleEventVibrateUseCase(intent.event, intent.enabled)
                    FetchWeather -> fetchWeather()
                    UpgradeToProRequest -> _sideEffect.send(RequestPurchase)
                    UpgradeToProIARequest -> _sideEffect.send(RequestSubscription)
        ...
        private fun scheduleImmediateEvents(events: List<Event>) {
            ...
            viewModelScope.launch {
                events.filter { it.isAlarmEnabled }.forEach { event ->
                    ...
                    val alarmFireTime = triggerTime ?: (event.startTime - TimeUnit.MINUTES.toMillis(alarmOffsetMinutes))
        ...
        private fun createEvent(intent: CreateEvent) {
            viewModelScope.launch {
                ...
                if (result != null) {
                    ...
                    refreshEvents()
                    _sideEffect.send(ShowSnackbar(StringResource(R.string.ai_agent_event_created)))
                } else {
                    _sideEffect.send(AIToolError(StringResource(R.string.ai_agent_event_creation_error)))
                }
            }
        }
