        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                if (!permissionManager.hasCalendarPermission()) {
                    logcat(LogPriority.WARN) { "Permissão READ_CALENDAR não concedida. O Worker não pode continuar." }
                ...
                logcat {
                    "Verificando eventos na janela de tempo: ${sdf.format(Date(now))} até ${sdf.format(Date(scheduleWindowEnd))}"
                }
                ...
                if (eventsToSchedule.isEmpty()) {
                    ...
                    eventsToSchedule.forEach { event ->
                        logcat { "Agendando alarme para o evento '${event.title}' em ${sdf.format(Date(event.startTime))}" }
